...
- [x] **Real-time Live Refresh:** Automatically refresh the process lists every 5 seconds without needing to click the "Scan" button manually.

### 🟡 Phase 2: Automation & Advanced Optimization
- [x] **Automated "Game Mode" Trigger:** Detect when a heavy game launcher (like Steam or Epic Games) opens, and automatically purge background junk in silence to maximize FPS.
- [x] **Game-Mode Exclusion List. A Exclusion List that you can select programs thats are running that may be closed when game-mode is enabled
